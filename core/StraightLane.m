classdef StraightLane
    ...
    % Assumption : The vehicle is driving from left to right on the lane and the lowest
    % lane is with the number id 1; the start position of the lowest line is (0,0)
    methods
        ...
        function [] = plot_lanes(self)
            for i = 1:self.num_lanes + 1
                ...
                % show the whole scenes in the visulization;
                % -10, 10, -2.5 and 2.5 are self defined values to make the visulization more clear
            end
        end
    end
end